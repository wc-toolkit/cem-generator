...
interface WebpackCompiler {
  ...
  hooks: {
    ...
    afterCompile: {
      tap(
        name: string,
        fn: (compilation: { fileDependencies: { add(path: string): void } }) => void,
      ): void;
    };
...
export class CemGeneratorWebpackPlugin {
  ...
  apply(compiler: WebpackCompiler): void {
    ...
    if (missing.length)
      console.warn(`[cem-generator] watchPaths do not exist: ${missing.join(", ")}`);
    ...
    compiler.hooks.watchClose.tap(CemGeneratorWebpackPlugin.#name, () =>
      this.#runner?.cancelScheduledRun(),
    );
    compiler.hooks.shutdown.tapPromise(CemGeneratorWebpackPlugin.#name, async () =>
      this.#runner?.cancelScheduledRun(),
    );
  }
}
