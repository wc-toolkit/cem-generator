...
test("detects FAST elements, custom element decorators, and @attr metadata", () => {
  ...
  assert.deepEqual(
    (booleanTest?.attributes ?? []).map((attribute) => attribute.name).sort(),
    ["normalAttr", "booleanAttr", "customName"].sort(),
  );
  assert.equal(
    booleanTest?.members?.some((member) =>
      ["connectedCallback", "disconnectedCallback", "attributeChangedCallback", "$emit"].includes(
        member.name,
      ),
    ),
    false,
  ...
  assert.equal(
    booleanTest?.members?.some((member) => member.name === "activate"),
    true,
  );
  ...
  assert.deepEqual(
    booleanTest?.cssParts?.map(({ name }) => name),
    ["indicator"],
  );
  assert.equal(booleanTest?.cssParts?.[0]?.description, "Indicator");
  assert.equal(booleanTest?.cssProperties?.[0]?.name, "--indicator-color");
  const importedStyles = declarations.find(
    (declaration) => declaration.name === "ImportedStylesFastElement",
  );
  assert.equal(importedStyles?.cssProperties?.[0]?.name, "--imported-indicator-color");
  assert.equal(
    manifest.modules
      .find((module) =>
        module.declarations.some((declaration) => declaration.name === "BooleanTest"),
      )
      ?.exports?.some(
        (entry) => entry.kind === "custom-element-definition" && entry.name === "boolean-test",
      ),
    true,
  );
});
