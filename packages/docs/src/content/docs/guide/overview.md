...
| **CSS-only elements**                  | Detects documented custom-element selectors in standalone CSS files, including `:is()`/`:where()`/`@scope` selectors, slots, attributes, public CSS custom properties, `@cssprop` tags, and `@attr` metadata. |
